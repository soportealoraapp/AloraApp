        ...
        <Card className="p-4 flex items-center gap-4 bg-muted/50 border-border">
            {recording ? (
                <div className="flex items-center gap-4 w-full justify-between">
                    <div className="flex items-center gap-2 text-destructive animate-pulse font-mono" role="status" aria-live="polite">
                        <div className="w-3 h-3 bg-destructive rounded-full"></div>
                        Rec {formatTime(duration)} / {formatTime(MAX_DURATION)}
                    </div>
                    <div className="flex gap-2">
                        {onCancel && (
                            <Button variant="ghost" size="icon" onClick={() => { stopRecording(); onCancel(); }} className="text-muted-foreground hover:text-destructive" aria-label="Cancelar grabación">
                                <Trash2 className="w-5 h-5" />
                            </Button>
                        )}
                        <Button variant="ghost" size="icon" onClick={stopRecording} className="bg-destructive/10 text-destructive hover:bg-destructive/20" aria-label="Detener grabación">
